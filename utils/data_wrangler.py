...
def strings_multi_cycle_features(num_cyles: int = 50) -> tuple[str]:
    """
    Create feature names corresponding to cycle number num_cycle
    """
    return (
        "f0",
        f"f{int(num_cyles / 2)}",
...
def gradient_estimate(f: np.ndarray, x: np.ndarray | None = None) -> np.ndarray:
    """
...
def ccv_signature_features(
    ...
    Returns:
    -------
            data frame of generated features | dict of ccv evolution.
    """

    ccv_multi_features: list[list[float]] = []
    ccv_dict: dict[str, list] = (
        {}
    )  # initialize a dict to store ccv for each cycle for all cells
    ...
    for cell in data_dict.keys():
        ...
        # get the multi-cycle features
        ccv_features = np.array(ccv_features)
        union = []
        for i in range(len(ccv_features[0])):
            union += multi_cycle_features(ccv_features[:, i], num_cycles)
