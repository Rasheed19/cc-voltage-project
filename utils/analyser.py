...
def format_prediction_and_interval(
    ...
    for i, cell in enumerate(sample_cells):
        ...
        ] = predictions["value_at_model"][i]

    for i, target in enumerate(target_list[:5]):
        prediction_df[f"{target} CI"] = prediction_intervals["cycle_at_model"][i]

    for i, target in enumerate(target_list[5:]):
        prediction_df[f"{target} CI"] = prediction_intervals["value_at_model"][i]
